...
def replace_from_yaml(
    ...
    Raises:
        FailToCreateError1 which holds list of `client.rest.ApiException`
    ...
    with open(path.abspath(yaml_file)) as f:
        ...
        for yml_document in yml_document_all:
            try:
                replace_from_dict(k8s_client, yml_document, verbose,
                                 namespace=namespace,
                                 **kwargs)
            except FailToCreateError1 as failure:
                failures.extend(failure.api_exceptions)
        if failures:
            raise FailToCreateError1(failures)


def replace_from_dict(k8s_client, data, verbose=False, namespace='default',
                     **kwargs):
    """
    ...
    if api_exceptions:
        raise FailToReplaceError(api_exceptions)

    return k8s_objects


def replace_from_yaml_single_item(
    ...
    if hasattr(k8s_api, "replace_namespaced_{0}".format(kind)):
        ...
        if "name" in yml_object["metadata"]:
            ...
            kwargs['name'] = name
        try:
            resp = getattr(k8s_api, f"replace_namespaced_{kind}")(
                body=yml_object, **kwargs)
        except client.rest.ApiException as api_exception:
            if api_exception.reason == 'Not Found':
                kwargs.pop('name', None)
                resp = getattr(k8s_api, f"create_namespaced_{kind}")(
                    body=yml_object, **kwargs)
            else:
                raise api_exception
    else:
        ...
        if "name" in yml_object["metadata"]:
            name = yml_object["metadata"]["name"]
            kwargs['name'] = name
        resp = getattr(k8s_api, "replace_{0}".format(kind))(
            body=yml_object, **kwargs)
    if verbose:
...
class FailToReplaceError(Exception):
    """
